...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* =============================================
           MARKER INSERT PDF (tidak terlihat secara visual,
...
<body>
    ...
    <?php foreach ($kategoriList as $idx => $kategori): ?>
        <h4 class="table-caption">Tabel <?= 6 + $idx ?>. Distribusi Soal <?= esc($kategori) ?></h4>
        <table>
            <thead>
                <tr>
                    <th>No. Soal</th>
                    <?php foreach ($cpmkData as $c): ?>
                        <th>CPMK <?= esc($c['no_cpmk']) ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
                <?php if (!empty($groupedSoal[$kategori])): ?>
                    <?php
                    $bySoal = [];
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="<?= count($cpmkData) + 1 ?>" class="center">Belum ada soal untuk kategori ini.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
    ...
    <p class="evaluasi-text">
        <?= nl2br(esc($portofolioData['isi_evaluasi'] ?? 'Belum ada evaluasi yang diisi.')) ?>
    </p>
